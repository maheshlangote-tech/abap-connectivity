...
/** \struct _RFC_JSON_OPTIONS
* \ingroup container
*
* Gives possible options to switch the Json format for RfcGetTableAsJson() and RfcGetStructureAsJson()
*/

typedef enum _RFC_JSON_OPTIONS {
	RFC_JSON_DEFAULT = 0,						///< Default
	RFC_JSON_OBJECT_ARRAY = 0x1,				///< A table row will be treated as array of objects
	RFC_JSON_OBJECT_ALL_STRING = 0x2			///< All values will be treated as string, in combination with RFC_JSON_OBJECT_ARRAY only. Might produce invalid Json format
}RFC_JSON_OPTIONS;
...
typedef enum _RFC_PROTOCOL_TYPE{
    ...
    RFC_CLOUD_CLIENT        ///< SAP Cloud Connector Client
}RFC_PROTOCOL_TYPE;
...
extern "C"
{
	 ...
	 * \return RFC_RC
	 */
	DECL_EXP RFC_RC SAP_API RfcSetTraceEncoding(const SAP_UC* traceEncoding, RFC_ERROR_INFO* errorInfo);
	...
	* \return RFC_RC
	*/
	RFC_RC SAP_API RfcUTF8ToSAPUC_CCE(const RFC_BYTE* utf8, unsigned utf8Length, SAP_UC* sapuc, unsigned* sapucSize, unsigned* resultLength, unsigned short onCCE, SAP_UINT substitute, RFC_ERROR_INFO* errorInfo);
	...
	*/
	...
	/**
	 * \brief  Opens an RFC client connection for invoking ABAP function modules in an ABAP backend.
	 * \ingroup connection
	 *
	 ...
	 *
	 * When using TCP socket communication to connect to a standalone server program, you need to provide the
	 * following connection parameters:
	 * - server_host, server_port
	 * and optionally the parameters for securing the connection with SSL/TLS:
	 * - ssl_cert_file, ssl_cert_password and ssl_partner_name
	 *
	...
	*
	* Standalone ("TCP Socket") Server:
	* - SERVER_PORT: specifies the port number on which the server is to listen
	* - BIND_ADDRESS: if the server is running on a multi-homed machine, you can optionally specify the IP address of the network card to use.
	* - REG_COUNT: maximum number of parallel requests that are allowed to be processed.
	* Note that a standalone server is always stateful.
	*
	* If you want the server to be secured with SSL/TLS, you have to provide the following additional parameters:
	* - SSL_CERT_FILE: a file in PKCS#12 format, containing the private key and the certificate chain to be used, beginning with the server's public
	*   certificate and ending with the root CA certifcate. It should also contain the CA certificates to be used for trust validation of the
	*   presented client certificates.
	* - SSL_CERT_PASSWORD: Optionally, the password (pin) for the PKCS#12 file.
	* - SSL_PARTNER_NAMES: DNs of client certificate's to be trusted.
	*
    ...
    */
	...
	/**
	 * \brief  Removes the RFC_SERVER_FUNCTION callback for a specific funcDescHandle that is installed globally or for a specific ABAP backend systemID.
	 * \ingroup installer
	 *
	 *
	 * \in *sysId System ID of the R/3 system, for which this function module implementation shall be removed.
	 * \in funcDescHandle A function description giving the name of the function module and its parameters.
	 * \out *errorInfo More details in case something goes wrong.
	 * \return RFC_RC
	 */

	DECL_EXP RFC_RC SAP_API RfcUninstallServerFunction(SAP_UC const* sysId, RFC_FUNCTION_DESC_HANDLE funcDescHandle, RFC_ERROR_INFO* errorInfo);
	...
	*/
    ...
    /**
    * \brief  Returns the value of the specified field as byte array starting from position "offset".
    * \ingroup container
    *
    * In case the buffer is too small, the function will return RFC_BUFFER_TOO_SMALL.
