 ...
 *
 * INPUT:
 * @param  length       Length of BCD buffer in bytes -> precision (2*length-1) or DECF_OVERFLOW
 * @param  decPlaces    Number of decimal places (Nachkommastellen) of the
 *                      BCD number
 * CHANGE:
 * @param  dfp16        Decfloat value
...
/**
 * \brief Round IEEE value such that it fits into ABAP type P (BCD) or HDB DEC(precision,decPlaces).
 *
 * If decFloatDBmode == DECF_DBmode_roundForDEC:
 * Round a 64-bit IEEE 754r decimal floating point value according to
 * decPlaces and check that it fits into the BCD type specified.
 ...
 *
 * INPUT:
 * @param  precision    max nr of digits of result, otherwise overflow, may be even or odd
 * @param  decPlaces    Number of decimal places (Nachkommastellen) of the
 *                      BCD number
 * CHANGE:
 * @param  dfp16        Decfloat value
 ...
 * @return DECF_INVALID_OP  Garbage input. The caller should raise an exception.
 */
DECL_EXP DECF_RETURN  SAP_API DecFloat16RoundForDECToPrecision(DecFloat16* dfp16, intR precision, intR decPlaces);
 ...
 * @return DECF_INVALID_OP  Garbage input. The caller should raise an exception.
 */
DECL_EXP DECF_RETURN  SAP_API DecFloat34RoundForDECToPrecision(DecFloat34* dfp34, intR precision, intR decPlaces);
 ...
 *
 * The following routines implement arbitrary precision arithmetic with decfloat.
 *
 * This means not decfloat16 or decfloat34, but decfloatN where N is arbitrary.
 */

enum SapDecfHighPrecOperation
{
  SapDecfHighPrecOp_Addition       = 0,
  SapDecfHighPrecOp_Subtraction    = 1,
  SapDecfHighPrecOp_Multiplication = 2,
  SapDecfHighPrecOp_Division       = 3,
  SapDecfHighPrecOp_Abs            = 4,
  SapDecfHighPrecOp_Sqrt           = 5,
  SapDecfHighPrecOp_Ln             = 6,
  SapDecfHighPrecOp_Log10          = 7,
  SapDecfHighPrecOp_Exp            = 8,
  SapDecfHighPrecOp_Pow            = 9
};

typedef SAP_RAW DecFloatHighPrec; ///!< unlimited precision decfloat number, stored in some internal format
                                  ///!< not exposed to the user of this library. Every high precision floating
                                  ///!< point number is an array of DecFloatHighPrec. The user must store the
                                  ///!< precision by himself. The length of the array is secret function of its
                                  ///!< precision. The precision is thus an input parameter of all functions.

typedef void * (*DecfHighPrecAllocCallBack)(SAP_ULLONG, void * data); ///!< callback used by some high precision
                                                                      ///!< arithmetic functions to allocate memory
 ...
 *
 * The result is that exact, in oposite to the current ABAP string division into decfloat34
 *
 * \in divident      Divident (as ASCII encoded string)
 * \in divisor       Divisor (as ASCII encoded string)
 * \in allocateFn    Callback to allocate memory for temporary values, caller frees memory
 * \in callBackData  User data (not used internally). Only given as parameter to the allocateFn callback
 * \out result       Result as Decfloat
 *
 * \ret              DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *                   All other -- Error!
 */
DECF_RETURN DecFloat34StrDiv(
  const SAP_A7 * divident,
  const SAP_A7 * divisor,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
  DecFloat34 * result);

/**
 * \brief Converts divident and divisor to an arbitrary precision decNumber format and
 * performs a division to a high precision decfloat number.
 *
 * \in divident      Divident (as ASCII encoded string)
 * \in divisor       Divisor (as ASCII encoded string)
 * \in allocateTmpFn allocation callback for temporary values
 * \in tmpCallBackData callback data for temporary values allocation callback
 * \in allocateFn    Callback to allocate memory for temporary values, caller frees memory
 * \in callBackData  User data (not used internally). Only given as parameter to the allocateFn callback
 * \out result       Result as high precision decfloat number
 *
 * \ret              DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *                   All other -- Error!
 */
DECF_RETURN DecFloatHighPrecStrDiv(
  const SAP_A7 * divident,
  const SAP_A7 * divisor,
  SAP_ULLONG precision,
  DecfHighPrecAllocCallBack allocateTmpFn,
  void * tmpCallBackData,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
  DecFloatHighPrec ** result);

/**
 * \brief Creates a high precision decfloat number from an ASCII encoded string. The precision
 * is given as input parameter.
 *
 * The high precision decfloat number is stored in an internal format.
 *
 *  \in number number encoded as ASCII
 *  \in precision target precision
 *  \in allocateFn callback to allocate memory for the resulting number
 *  \in callBackData user proviced data. Not used internally and is only forwarded to the allocate callback
 *  \out result Result pointer. It's content is set to the resulting number.
 *  \ret             DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *                   All other -- Error!
 */
DECF_RETURN DecFloatHighPrecFromAscii(
  const SAP_A7 * number,
  SAP_ULLONG precision,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
...
/**
 * \brief Convert a high precision decfloat number to ASCII string.
 *
 * The high precision decfloat number is stored in an internal format.
 *
 *  \in number number encoded as ASCII
 ...
 *  \in allocateFn callback to allocate memory for the resulting number
 *  \in callBackData user proviced data. Not used internally and is only forwarded to the allocate callback
 *  \out resultString resulting string
 *  \ret  DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecToAscii(
  const DecFloatHighPrec * number,
  SAP_ULLONG precision,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
  SAP_A7 ** resultString);
...
/**
 * \brief Convert an integer to a high precision decfloat number,
 *
 * The high precision decfloat number is stored in an internal format.
 *
 *  \in number number
 ...
 *  \in allocateFn callback to allocate memory for the resulting number
 *  \in callBackData user proviced data. Not used internally and is only forwarded to the allocate callback
 *  \out result resulting high precision number
 *  \ret  DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecFromInt(
  SAP_INT number,
  SAP_ULLONG precision,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
...
/**
 * \brief Convert a decfloat34 to a high precision decfloat number,
 *
 * The high precision decfloat number is stored in an internal format.
 *
 *  \in number number
 ...
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecFromDecFloat34(
  const DecFloat34 * number,
  SAP_ULLONG precision,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
  DecFloatHighPrec ** result);
...
/**
 * \brief Performs a computation with two operands and store the result in a third operand.
 * The result can be the first operand, in which case no memory is allocated and the precision
 * of the first operand and the result must be identical.
 *
 *  \in firstOperand first operand
 *  \in secondOperand second operand
 *  \in firstPrecision precision of the first operand
 *  \in secondPrecision precision of the second operand
 *  \in precision precision of the target operand
 *  \in allocateFn callback to allocate memory for the resulting number
 *  \in callBackData user proviced data. Not used internally and is only forwarded to the allocate callback
 *  \in operation computation to be performed
 *  \in resultOperandIsFirstOperand set to true, if the first operand and the result are the same
 *  \out result resulting high precision number
 *  \ret  DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecOperation(
  const DecFloatHighPrec * firstOperand,
  const DecFloatHighPrec * secondOperand,
  SAP_ULLONG firstPrecision,
  SAP_ULLONG secondPrecision,
  SAP_ULLONG precision,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
  enum SapDecfHighPrecOperation operation,
  SAP_BOOL resultOperandIsFirstOperand,
  DecFloatHighPrec ** result);

/**
 * \brief Compare a high precision decfloat number with another.
 *
 *  The result is -1 (smaller), 0(equal) or 1(larger)
 *
 *  \in firstOperand first operand
 *  \in firstPrecision precision of first operand
 *  \in secondOperand second operand
 *  \in secondPrecision precision of first operand
 *  \out resultInt result of the comparison
 *  \ret  DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecCompare(
  const DecFloatHighPrec * firstOperand,
  SAP_ULLONG firstPrecision,
  const DecFloatHighPrec * secondOperand,
  SAP_ULLONG secondPrecision,
...
/**
 * \brief Compare a high precision decfloat number with an integer.
 *
 *  The result is -1 (smaller), 0(equal) or 1(larger)
 *
 *  \in firstOperand first operand
 ...
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecCompareInt(
  const DecFloatHighPrec * firstOperand,
  SAP_ULLONG firstPrecision,
  SAP_INT secondOperand,
  SAP_INT * resultInt);

/**
 * \brief Rescales the decfloat number, so that the exponent equals the given
 * integer. i.e. number "1" and exponent "-1" gives "10e-1".
 *
 *  \in operand number
 *  \in operandPrecision precision of number
 *  \in targetPrecision precision of target number
 *  \in allocateFn callback to allocate memory for the resulting number
 *  \in callBackData user proviced data. Not used internally and is only forwarded to the allocate callback
 *  \in exponent exponent
 *  \in operandIsResult operand should be equal to the result
 *  \out result result of the computation
 *  \ret  DECF_OK, DECF_INEXACT, DECF_UNDERFLOW -- Fine.
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecRescale(
  const DecFloatHighPrec * operand,
  SAP_ULLONG operandPrecision,
  SAP_ULLONG targetPrecision,
  DecfHighPrecAllocCallBack allocateFn,
  void * callBackData,
  SAP_INT exponent,
  SAP_BOOL operandIsResult,
  DecFloatHighPrec ** result);

/**
 * \brief Get the exponent of the decfloat number, i.e. 1e12 yields 12.
 *
 *  \in operand number
 *  \in operandPrecision precision of number
 ...
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecGetExponent(
  const DecFloatHighPrec * operand,
  SAP_ULLONG operandPrecision,
  SAP_LLONG * exponent);

/**
 * \brief Get the digits of the decfloat number, i.e. 1e12 yields 1.
 *
 *  \in operand number
 *  \in operandPrecision precision of number
 *  \out digits digits
 ...
 *        All other -- Error!
 */
DECF_RETURN DecFloatHighPrecGetDigits(
  const DecFloatHighPrec * operand,
  SAP_ULLONG operandPrecision,
  SAP_LLONG * digits);

/**
 * \brief Converts a high precision number to decfloat34
 *
 *  \in operand number
 *  \in precision precision of number
 *  \out result resulting number
 */
DECF_RETURN DecFloatHighPrecTo34(
  const DecFloatHighPrec * operand,
  SAP_ULLONG precision,
  DecFloat34  * result);
