...
#define SAP_U16_PROTOTYPE_HLP(nameU16)  externC DECL_EXP nameU16##_RETURN nameU16 nameU16##_TPARAMS;
  ...
  #define SAP_U16_PROTOTYPE_UX_HLP(nameU16)
#endif
#define SAP_U16_PROTOTYPE_STDC_HLP(nameU16) SAP_U16_PROTOTYPE_HLP(nameU16)
...
/*-----------------------------------------------------------------------------
 * sapuc.h  -  File identification (SAP_IDENTN)
 *----------------------------------------------------------------------------*/
/* There are three uses cases currently observed in the kerne source code:
 * (1) In a file including saptype.h or sapuc.h
 *     the symbols sccsid (non-Unicode) and sccsidU (common UC/NUC semantics) are
 *     defined in sapuc.h using the COMPILE_UNIT_REL_PATH compile flag. Since git
 *     does not support RCS keyword substitution, the sccsid(U) strings do not contain
 *     version information anymore.
 * (2) In a source file including only saptype.h use
 *     SAP_IDENTN(name,"@(#) $Id$")
 *     after(!) including saptype.h to define a identification string explicitely.
 ...
 * (3) If you want to set the sccsid(U) string explicitely, e.g. in case you want
 *     to add some compile flag information (e.g. "DEBUG") to the sccsid(U) string.
 *     Then define the macro SAP_IDENT_FOLLOWS before(!) including sapuc.h.
 *     The definition of SAP_IDENT_FOLLOWS prevent the definition of the sccsid(U)
 *     symbol in sapuc.h, otherwise you would get duplicate symbol linker errors.
 * The macro SCCSID and SAP_IDENT are obsolete and must not be used anymore.
 * It is still possible to define several SAP_IDENTN (e.g. for includes) with different
...
SAP_U16_PROTOTYPE_MODE_T(mkdir)
  ...
  #define readlinkU16_TPARAMS         (const SAP_UTF16 *path, SAP_UTF16 *buf, size_t size)
...
#define putsR( s )                     /*SAPUNICODEOK_LIBFCT*/ /*CCQ_RAWDECL*/puts( s )
